...
public class Grafo <T> {
    ...
    if (index != -1) {
      ...
      for (int v = 0; v < LVertices.size(); v++) {
        for (int a = 0; a < LVertices.get(v).getAdyacentes().size(); a++) {
          if (LVertices.get(v).getAdyacentes().get(a).getCveV() == cveVer) LVertices.get(v).getAdyacentes().remove(a);
        }
      }
    } else throw new GrafoException("Vertice no encontrado.");
  }

  public void addArista(int cveVerOri, int cveVerDes, DatosA datos) {
    int indexO = IndexOfV(cveVerOri);
    int indexD = IndexOfV(cveVerDes);
    if (indexO != -1 && indexD != -1) LVertices.get(indexO).getAdyacentes().add(new Arista < DatosA > (cveVerDes, datos));
    if (!Diri) LVertices.get(indexO).getAdyacentes().add(new Arista < DatosA > (cveVerOri, datos));
  }
      ...
      if (indexD != -1) {
        ...
        if (!Diri) {
          ...
          if (indexO != -1) {
            ...
            if (indexD != -1) LVertices.get(indexO).getAdyacentes().remove(indexD);
          } else throw new GrafoException("El vertice no existe.");
        }
      }
    } else throw new GrafoException("El vertice no existe");
  }
  ...
  private int IndexOfV(int cve) {
    ...
    if (LVertices.get(i).getCve() == cve) return i;
    return - 1;
  }

  private int IndexOfA(ArrayList < Arista < DatosA >> LA, int cve) {
    for (int i = 0; i < LA.size(); i++)
    if (LA.get(i).getCveV() == cve) return i;
    return - 1;
  }
  ...
  private void RPA() {
    ...
    if (LVertices.size() > 0) {
      ...
      System.out.println();
    } else System.out.println("Grafo vacio.");
  }
  ...
  private void RPP(int index) {
    ...
    for (int i = 0; i < LVertices.get(index).getAdyacentes().size(); i++) {
      ...
      if (!LVertices.get(SigIndex).isVisitado()) RPP(SigIndex);
    }
  }
  ...
  private void preparaInicioRecorrido() {
    for (int i = 0; i < LVertices.size(); i++)
  ...
  public Vertice < DatosV > getVertice(int CveVert) {
    return LVertices.get(IndexOfV(CveVert));
  }
    ...
    if (LVertices.size() > 0) {
      for (int v = 0; v < LVertices.size(); v++) {
        ...
        if (LVertices.get(v).getAdyacentes().size() > 0) {
          for (int a = 0; a < LVertices.get(v).getAdyacentes().size(); a++) {
            Salida += LVertices.get(v).getAdyacentes().get(a).getCveV() + ": " + LVertices.get(IndexOfV(LVertices.get(v).getAdyacentes().get(a).getCveV())).getDatos().getCiudad() + ", ";
          }
        }
        Salida += '\n';
      }
